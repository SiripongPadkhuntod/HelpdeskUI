...
const ManageTicketsPage = ({ tickets = [] }) => {
  const [ticketList, setTickets] = useState(tickets);
  ...
  const updateTicketDetails = async (id, details) => {
    try {
      ...
      setTickets(ticketList.map((ticket) => (ticket.id === id ? { ...ticket, ...details } : ticket)));
    } catch (error) {
      console.error('Error updating ticket: ', error);
    }
  };
  ...
  const onDragEnd = (result) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      const ticket = ticketList.find((ticket) => ticket.id === result.draggableId);
      if (ticket) {
